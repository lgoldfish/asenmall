   ...
   Media.find({}).sort({"_id":-1}).then((data3)=>{
    ...
    Banner.find().then((data1) => {
      ...
      // console.log(mediapic)
            // 遍历筛选数据库数据(商品数据)
            var zncs=[];var hjsb=[];var qgsb=[];var dhj=[];var fjl=[];
            var hjpj=[];var qgpj=[];var ldbh=[];var gywj=[];var hc=[];
            for(var i=0;i<data2.length;i++){
              switch (data2[i].id){
              case "智能仓储":zncs.push(data2[i]);break;
              case "焊接设备":hjsb.push(data2[i]);break;
              case "切割设备":qgsb.push(data2[i]);break;
              case "电焊机":dhj.push(data2[i]);break;
              case "辅具类":fjl.push(data2[i]);break;
              case "焊接配件":hjpj.push(data2[i]);break;
              case "切割配件":qgpj.push(data2[i]);break;
              case "劳动保护":ldbh.push(data2[i]);break;
              case "工业五金":gywj.push(data2[i]);break;
        ...
        Commodity.find({ id: lists }).sort({ _id: -1 }).then((data3) => {
         ...
         if(req.query.fromvue){
            ...
            res.render("list", {
                banner: data,
                commodity:data3,
                listname:lists,zncs,hjsb,qgsb,dhj,fjl,hjpj,qgpj,ldbh,gywj,hc
            })
         }
        }, (err) => {
            console.log("err", err)
    ...
    Banner.find().then((data) => {
        Commodity.count().then((aCount) => {
            ...
            res.render("allList", {
                ...
                zncs,hjsb,qgsb,dhj,fjl,hjpj,qgpj,ldbh,gywj,hc,
                count:aCount,
                commodityCount:Math.ceil(aCount/pageCount),currentPage:page
            })
        }, (err) => {
            console.log("err", err)
        })
